...
class AuthController extends Controller
{
    ...
    {
        return app(RateLimiter::class)->tooManyAttempts($request->input($this->loginUsername()).$request->ip(), $this->maxLoginAttempts());
    }
    ...
    {
        app(RateLimiter::class)->hit($request->input($this->loginUsername()).$request->ip(), $this->lockoutTime() / 60);
    }
    ...
    protected function sendLockoutResponse(Request $request)
    {
        ...
        return redirect('login')->withInput($request->only($this->loginUsername(), 'remember'))->withErrors([
    ...
    public function postRegister(RegisterRequest $request, RoleRepository $roles)
    {
        ...
        $user = $this->users->create(array_merge($request->only('email', 'username', 'password'), ['status' => $status, 'role_id' => $role->id]));
    ...
    public function confirmEmail($token)
    {
        ...
        if ($user = $this->users->findByConfirmationToken($token)) {
            ...
            return redirect()->to('login')->withSuccess(trans('app.email_confirmed_can_login'));
        }

        return redirect()->to('login')->withErrors(trans('app.wrong_confirmation_token'));
    }
}
