...
/*
 * Custom TMRemote.io API
 * todo: Add a setting to enable/disable gm status
 */
//if (settings('2fa.enabled')) {
Route::group(['middleware' => ['auth', 'permission:gm.status']], function () {
    Route::get('gm/status', 'GmStatusController@status');
});
Route::post('gm/log', 'GmStatusController@log');
//}

Route::group(['middleware' => ['auth', 'permission:gm.status']], function () {
    Route::post('bots/log', 'BotsController@log');
});
