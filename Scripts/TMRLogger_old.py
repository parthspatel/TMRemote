...
class Log(object):
    ...
    def LogGM():
        try:
            worldStatus = ast.literal_eval(Log.ReadGMLogs())[GetWorld(Terminal.GetComboBox('LoginWorld'))]
            worldsToCheck = ast.literal_eval(Log.ReadWorldsToLogOut())[GetWorld(Terminal.GetComboBox('LoginWorld'))]
    ...
    def Items():
        try:
            IGN = Log.removeAccents(Character.GetName())
        except:
            IGN = 'N/A'
        ...
        try:
            Log.toFile('TMRemote/logs', 'ab', items)
            return True
        except:
            try:
