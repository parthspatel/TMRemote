...
class Log(object
    ...
    def LogGM():
        try:
            worldStatus=ast.literal_eval(Log.ReadGMLogs())[
                GetWorld(Terminal.GetComboBox('LoginWorld'))]
            worldsToCheck=ast.literal_eval(Log.ReadWorldsToLogOut())[
                GetWorld(Terminal.GetComboBox('LoginWorld'))]
    ...
    def Items():
        ...
        try:
            Log.toFile('TMRemote/temp/logs', 'ab', items)
            return True
        except:
            try:
